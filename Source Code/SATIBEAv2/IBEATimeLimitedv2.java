...
public class IBEATimeLimitedv2 extends Algorithm {
     ...
     */
    public IBEATimeLimitedv2(Problem problem, long maxRunTimeMS,SAT_Decision sat_dec,RichSeedGen richsgen) {
        super(problem);
        this.maxRunTimeMS = maxRunTimeMS;
        this.sat_dec = sat_dec;
        this.richsgen = richsgen;
    } 
...
//        }
    ...
    //判断一个字符串中有多少个0
    public int returnZero(String str){
    	int count=0;
    	for(int i=0;i<str.length();i++){	//通过length方法获取字符串长度
    	    if(str.charAt(i)=='0')			//charAt是取字符串第i个字符
    	    	count++;
    	}
    	return count;
    }
    ...
    public SolutionSet execute() throws JMException, ClassNotFoundException {
        ...
        //判断哪个Solution
        int zeroCountTem=0,stdIndex=0;
        for(int std =0;std<solutionSet.size();std++)
        {
        	zeroCountTem = returnZero(solutionSet.get(std).getDecisionVariables()[0].toString());
        	if(zeroCount < zeroCountTem)
        	{	
        		zeroCount = zeroCountTem;
        		stdIndex = std;
        	}
        }

        solutionSet.get(stdIndex).setDecisionVariables(this.richsgen.richSeedGeneration());
        problem_.evaluate(solutionSet.get(stdIndex));
        problem_.evaluateConstraints(solutionSet.get(stdIndex));
        evaluations++;
        solutionSet.add(stdIndex, solutionSet.get(stdIndex));
        ...
        while (elapsed < this.maxRunTimeMS) {
            ...
            while (offSpringSolutionSet.size() < populationSize) {
                ...
                } while (j < IBEATimeLimitedv2.TOURNAMENTS_ROUNDS); // do-while
                int k = 0;
                do {
                    k++;
                    parents[1] = (Solution) selectionOperator.execute(archive);
                } while (k < IBEATimeLimitedv2.TOURNAMENTS_ROUNDS); // do-while
