...
import satibeaVariants.SATIBEA_SettingsIBEAv4;

public class SATIBEAv4_Variants_Main{
    ...
    public static void main(String[] args) throws Exception {
    	...
    	try{
            ...
            /*************/
            String ResultDirectory = "SATIBEAv4_result";
            ...
            for(int count = 0;count<program_timeout_ms.length;count++)
            {
            	...
            	for(int i = 0; i< systems.length; i++){
            		...
            		while(loop > 0){
            			/********************/
            			outputFileSATibea = Outpath + systems[i] + ".SATibeav4-"+(program_timeout_ms[count]/6000)+"m.out." + loop;

            			p = new SATIBEA_Problem(fm, augment, mandatory, dead, seed);
            			a = new SATIBEA_SettingsIBEAv4(p).configureSATIBEAv4(program_timeout_ms[count], fm, 
            					((SATIBEA_Problem) p).getNumFeatures(), 
            					((SATIBEA_Problem) p).getConstraints(),outputFileSATibea);
            			/********************/
            			long start = System.currentTimeMillis();
            			SolutionSet pop = a.execute();
            			System.out.println("evaluation times：SATibeav4-"+(program_timeout_ms[count]/6000)+"m_"+
