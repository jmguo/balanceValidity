 ...
 * @author chris
 */
public class IBEATimeLimitedAmazon extends Algorithm {
     ...
     */
    public IBEATimeLimitedAmazon(Problem problem, long maxRunTimeMS,SAT_Decision sat_dec) {
        super(problem);
...
//        }
    ...
    public SolutionSet execute() throws JMException, ClassNotFoundException {
        ...
        /******************************/
        HashSet<Integer> blacklist= new HashSet<Integer>();//用于存放违背约束的位置
        Binary bin;
        int violated;
        /******************************/
        ...
        for(int soi=0; soi < solutionSet.size(); soi++)
        {
        	...
        	for(int ij=0;ij<Ssolution.getDecisionVariables().length; ij++)
        	{
        		...
        		if (violated > 0) {
        			...
        			for (int j = 0; j < SATIBEA_Problem_Amazon.numFeatures; j++) {
        				int feat = j + 1;
        ...
        while (elapsed < this.maxRunTimeMS) {
            ...
            while (offSpringSolutionSet.size() < populationSize) {
                ...
                } while (j < IBEATimeLimitedAmazon.TOURNAMENTS_ROUNDS); // do-while
                int k = 0;
                do {
                    k++;
                    parents[1] = (Solution) selectionOperator.execute(archive);
                } while (k < IBEATimeLimitedAmazon.TOURNAMENTS_ROUNDS); // do-while

                //make the crossover
                Solution[] offSpring = (Solution[]) crossoverOperator.execute(parents);//交叉
                mutationOperator.execute(offSpring[0]);//突变
                /*****************************************************/
                //重新进行约束不满足修复
                blacklist = new HashSet<Integer>();
            	bin = (Binary) offSpring[0].getDecisionVariables()[0];
            	violated = this.sat_dec.numViolatedConstraints(bin, blacklist);
                if(violated!=0)
                {
                	for(int ijc=0; ijc < offSpring[0].getDecisionVariables().length; ijc++)
                	{
                		bin = (Binary) offSpring[0].getDecisionVariables()[ijc];
                		violated = this.sat_dec.numViolatedConstraints(bin, blacklist);
                		if (violated > 0) {
                			IVecInt iv = new VecInt();
                			for (int jc = 0; jc < SATIBEA_Problem_Amazon.numFeatures; jc++) {
                				int feat = jc + 1;
                				if (!blacklist.contains(feat)) {
                					iv.push(bin.bits_.get(jc) ? feat : -feat);
                				}
                			}
                			boolean[] prod = this.sat_dec.randomProductAssume(iv);
                			for (int jc = 0; jc < prod.length; jc++) {
                				bin.setIth(jc, prod[jc]);
                			}
                		}
                	}
                }
