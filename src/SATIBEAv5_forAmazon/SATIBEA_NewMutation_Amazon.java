...
public class SATIBEA_NewMutation_Amazon extends Mutation {
    ...
    private static final List VALID_TYPES = Arrays.asList(BinarySolutionType.class,
            ...
            IntSolutionType.class, SATIBEA_BinarySolution_Amazon.class);
     ...
     */
    public SATIBEA_NewMutation_Amazon(HashMap<String, Object> parameters, String fm, int nFeat, List<List<Integer>> constraints,SAT_Decision sat_dec) {
        super(parameters);
        ...
        if (false) {
            try {
                if ((solution.getType().getClass() == BinarySolutionType.class)
                        ...
                        || solution.getType().getClass()==SATIBEA_BinarySolution_Amazon.class) {
                    for (int i = 0; i < solution.getDecisionVariables().length; i++) {
                        //for (int j = 0; j < ((Binary) solution.getDecisionVariables()[i]).getNumberOfBits(); j++) {
                        for (Integer j : SATIBEA_Problem_Amazon.featureIndicesAllowedFlip) { //flip only not "fixed" features
