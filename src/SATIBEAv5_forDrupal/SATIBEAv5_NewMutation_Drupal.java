...
public class SATIBEAv5_NewMutation_Drupal extends Mutation {
    ...
    private static final List VALID_TYPES = Arrays.asList(BinarySolutionType.class,
            ...
            IntSolutionType.class, SATIBEA_BinarySolution_Drupal.class);
     ...
     */
    public SATIBEAv5_NewMutation_Drupal(HashMap<String, Object> parameters, String fm, int nFeat, List<List<Integer>> constraints, SAT_Decision sat_dec2) {
        super(parameters);
        ...
        if (false) {
            try {
                if ((solution.getType().getClass() == BinarySolutionType.class)
                        ...
                        || solution.getType().getClass()==SATIBEA_BinarySolution_Drupal.class) {
                    for (int i = 0; i < solution.getDecisionVariables().length; i++) {
                        //for (int j = 0; j < ((Binary) solution.getDecisionVariables()[i]).getNumberOfBits(); j++) {
                        for (Integer j : SATIBEA_Problem_Drupal.featureIndicesAllowedFlip) { //flip only not "fixed" features
